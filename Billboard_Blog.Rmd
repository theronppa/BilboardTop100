...
for (row in 1:nrow(all_data_lyrics)) {
  ...
  if (grepl("featuring", all_data_lyrics[[3]][row])) {
  ...
  }else if (grepl("and", all_data_lyrics[[3]][row])) {
    ...
    genre <- searchArtist(all_data_lyrics[[3]][row],token = my_oauth)[5]$genres[1]
    if (is.null(genre)) {
      artist_genre[[row]] <- NA
    }else {
      artist_genre[[row]] <- sub("\\,.*", "", genre)
...
Once we were able to get a genre for each artist, then we could generalize these into 8 neat litle categories.
...
Word clouds are always a good tool to visualize all the facts. By unnesting the different words within the lyrics column and transforming it into a document term matrix we were able to create a dataset consisting of only the most prominently used words.
```{r eval = FALSE}
...
In order to answer this question we had to  < add christiaan part here >
...
After applying Luvain community detection 
```{r}
