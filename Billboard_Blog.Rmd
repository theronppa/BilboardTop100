...
It was identifed that there was over 800 different genres making it difficult to deduce significant results from the dataset.
...
Another challenge was collecting lyrics for songs that were not available on the original dataset - so we had to pull it from an external source via a scraper. 

*Generalizing Genres*
As genres are an important aspect of this analysis - we had to pull in genres from a Spotify API, and generalise them. This took some time to sort them into generalizations and checking smaller samples to ensure the results made sense. 
...
## Question 2.1 : 
Most promonent curse  words in lyrics. What is a song without some explicity? The following word cloud displays the most prominent swearwords. The swearword dictionary was provided by yours truly.
...
What is the most likely element of a song that brings us to our feet other than it being written by Neil Diamond and titled as an agreeable popular female name? WHO KNOWS! But we'll take a wild swing at Tempo for the time being. 
...
```{r echo=FALSE, fig.height=8, fig.width=8, warning=}
...
It was assumed that collaborations would primarily occur in the same genre. We thus ran the `Louvain community detection` algorithm to distinguish the different artists in the defined communities. The reason why we choose the Louvain method is because of its fast processing time, responsiveness and accuracy on bigger datasets like this one.
...
The graph  above was coloured according to different communities. 
Originally there was 100 different identified communities but for visualization purposes we filtered for the 9  biggest communities. 
Upon observation, there definitely seems to be a distinction between where artists primarily collaborate. With many of them staying within their communities and a few branching out to surrounding  communities. 


*Changing Edge colors to reveal actual gendres*


Looking back at our question,  we need to determine if this graph does in fact correlate to genres. 
By just looking at graph, the it does seem that similar artists group together (like that of Drake, Kanye West and DJ Khaled ). This could be leaning in favour of our assumption. To truly see wether these communities are show casting  genres we needed to extract the genre ratios found in each community to determine wether one genre is promonent within the communities.
For our assumption to be true we would have like to see a higher correlation with the ratio of one genres per community and the paramenters of a community. 


To do this we took our `collaboration dataset` and commaring the edge and node tables determined:
1. Which genres where in a give community &
2. What were the genres ratio proportinal to the communites size. 

*Our findings:*
From the 100 identified communities 70% of them had one primary genre and 94%  of the communities had a genre that made up more than 50% of the communities size.  This is great news as it shows how the community detection and genres had somthing incommon when it came to collaborating artist. 
Below are a few samples from the Dataset and their respective genre ratios. 
```{r echo=FALSE, out.width='25%'}
knitr::include_graphics('DiagramsGephi/100.png') 
knitr::include_graphics('DiagramsGephi/mix.png') 
knitr::include_graphics('DiagramsGephi/87.png') 
```

From this we can see how the majority of the communities that were detected had the  genres > this could aid in supporting the assumption that collaborations primarily occur within communities.>

From this we can concluded that the factors that were leading groups to  collaborate in communities could definitly be linked to the genres of those artists. 
...
We were able to observe such an ocurrence in Drake’s pop community.
As you can see by the weighting in the edges( which were weighted according to the amount of collaboration between nodes),  artists seem to constantly collaborate within their communities with a few of them branching out into different communities. 
Here, Rihanna seems to be playing a strong bridging role in her community exposing it to more genres and possibily making it easier for artists inside her community to do future collaborations through her existing ties.
