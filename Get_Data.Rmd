...
for (row in 1:nrow(lyrics_added)) {
  print(row)
  if (grepl("featuring", lyrics_added[[3]][row])) {
    
    z <- gsub("featuring.*","",lyrics_added[[3]][row])
    genre <- searchArtist(substr(z, 1, nchar(z)-1), token = my_oauth)[[5]][1]
    
    if (is.null(genre)) {
      if (grepl("and", lyrics_added[[3]][row])) {
        
        x <- gsub(" and.*","",lyrics_added[[3]][row])
        x[] <- lapply(x, gsub, pattern='"', replacement='')
        genre <- searchArtist(x[[1]], token = my_oauth)[[5]][1]
  ...
  }else if (grepl("and", lyrics_added[[3]][row])) {
    ...
    genre <- searchArtist(lyrics_added[[3]][row],token = my_oauth)[[5]][1]
    if (is.null(genre)) {
      artist_genre[[row]] <- NA
    }else {
      artist_genre[[row]] <- sub("\\,.*", "", genre)
...
df_all <- data_with_genre
df_all$genre <- ifelse(grepl(paste(pop, collapse = "|"), asd$genre_data_2017), "pop",
...
df_all$genre_data_2017 <- NULL

saveRDS(df_all, file = "data_in/all_data_lyrics_genre.rds")
```
