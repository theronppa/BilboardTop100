...
corpus_artist <- Corpus(VectorSource(ranked_data$artist))
dtm_artist <- TermDocumentMatrix(corpus)

m_artist <- as.matrix(dtm_artist)
v_artist <- sort(rowSums(m_artist),decreasing=TRUE)
d_artist <- data.frame(word = names(v_artist),freq=v)
